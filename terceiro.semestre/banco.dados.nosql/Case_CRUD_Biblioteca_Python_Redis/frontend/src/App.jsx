...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Se chegou notificação nova não lida, mostra toast
    ...
    poll();
    const id = setInterval(poll, 20000); // a cada 20s
  ...
  return (
    <Box sx={{ bgcolor: "background.default", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      {/* Área principal: sempre grande o suficiente para o footer ficar abaixo da tela */}
      ...
      >
        © {new Date().getFullYear()} Biblioteca Digital · React + MUI + Flask + Redis
      </Box>
